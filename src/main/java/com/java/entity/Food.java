...
"FROM " +
"(SELECT ROWNUM rnum, A.* FROM  " +
"(SELECT food_code, food_name, food_addr, food_area, food_phone, food_kind, food_read, food_menu, image_name, " + 
"image_path, nvl(review_score,0) review_score, nvl(review_count,0) review_count FROM  " +
...
"else 4 end asc) fo, image, (SELECT food_code review_food, NVL(AVG(review_score),0) review_score, " + 
"nvl(COUNT(review_code),0) review_count FROM review GROUP BY food_code) rv  " +
"WHERE fo.food_code = image.refer_code(+)  " +
"AND fo.food_code = rv.review_food(+)  " +
"ORDER BY food_read DESC )A) WHERE rnum >= :startRow and rnum <= :endRow ",
