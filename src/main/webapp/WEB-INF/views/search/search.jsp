...
<div id="content">
	...
	<div class="food_nav_2">
			...
			function mark(addr, name, filter, foodCode, imageName){
			    ...
			    // 정상적으로 검색이 완료됐으면 
			     if (status === kakao.maps.services.Status.OK) {
					...
					var foodImage = "${root}/static/ftp/"+imageName;
		...
		<div style=" margin: 15px 30px; font-size: 28px; color: #309cef;">관련 쿠폰</div>
		...
		<%-- <div class="couponNav">
			...
			<div id="inner" class="couponList" style="cursor:pointer;" onclick="location.href='${root}/food/read.go?foodCode=${couponDto.foodCode}'">
				<div style="background-image: url('${root}/static/ftp/${couponDto.imageName}'), url('${root}/static/css/noimage.jpg');">
					<img alt="쿠폰 이미지" src="${path}${couponDto.imageName}" onerror="this.src='${root}/static/css/noimage.jpg'">
		...
		<div class="couponNav">
			...
			<div id="inner" class="couponList" style="cursor:pointer;" onclick="location.href='${root}/food/read.go?foodCode=${couponDto.foodCode}'">
				<div>
					<img alt="쿠폰 이미지" src="/static/ftp/${couponDto.imageName}" onerror="this.src='${root}/static/css/noimage.jpg'"> 
					<span>${couponDto.couponName}</span>
				</div>
				<div>
...
<!-- 					<div>
					...
					<div class="container_1" >
						<div class="radio-tile-group">
							<div class="input-container">
								<input id="kindType1" class="radio-button" type="radio" name="kindType" value="한식" />
								<div class="radio-tile">
								<label for="kindType1" class="radio-tile-label" style="font-size: 15px;">한식</label>
								</div>
							</div>
							<div class="input-container">
								<input id="kindType2" class="radio-button" type="radio" name="kindType" value="중식" />
								<div class="radio-tile">
								<label for="kindType2" class="radio-tile-label" style="font-size: 15px;">중식</label>
								</div>
							</div>
							<div class="input-container">
								<input id="kindType3" class="radio-button" type="radio" name="kindType" value="일식" />
								<div class="radio-tile">
								<label for="kindType3" class="radio-tile-label" style="font-size: 15px;">일식</label>
								</div>
							</div>
							<div class="input-container">
								<input id="kindType4" class="radio-button" type="radio" name="kindType" value="양식" />
								<div class="radio-tile">
								<label for="kindType4" class="radio-tile-label" style="font-size: 15px;">양식</label>
								</div>
							</div>
							<div class="input-container">
								<input id="kindType5" class="radio-button" type="radio" name="kindType" value="카페" />
								<div class="radio-tile">
								<label for="kindType5" class="radio-tile-label" style="font-size: 15px;">카페</label>
								</div>
							</div>
							<div class="input-container">
								<input id="kindType6" class="radio-button" type="radio" name="kindType" value="기타" />
								<div class="radio-tile">
			...
			if (resultCount > 0) {
				for (var i = 0; i < data.length; i++) {
					...
					// onclick='location.href=\""+url+"\"'>"; -> 해당 foodcode 음식점으로 이동
						cont += "<div>";
							var err = root + "/static/css/noimage.jpg";
							// 이미지 없을때
							cont += "<img alt='음식 이미지' src='/static/ftp/"+data[i].imageName+"' onerror='this.src=\""+err+"\"'>";
						cont += "</div>";
						cont += "<div>";
							// 음식점 등록할 시 작성한 이름
...
$(function() {
	if (kind != null && kind != "") {
		$('input:radio[name=kindType]:input[value='+kind+']').attr("checked", true);
		searchType();
	}
	searchCountAjax();
	searchResultAjax();
