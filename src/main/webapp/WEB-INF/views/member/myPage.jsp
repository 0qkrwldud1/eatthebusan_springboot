				...
				<div class="review_1" style="cursor: pointer;" onclick="location.href='${root}/food/read.go?foodCode=${reviewList.foodCode}'">
					...
					<div>
						...
						<c:forEach var="imgList" items="${img}">
							<img src="/static/ftp/${imgList}" alt="이미지 준비중" style="width: 100px; height: 100px;" />
						</c:forEach>
					</div>
				</div>
				<div class="review_2">
			...
			<div class="coupon_2">
				...
				<c:choose>
					<c:when test="${couponList.purchaseStatus == 'Y'}">
						<ul>
							<li><img alt="쿠폰" src="${root}/static/ftp/${couponList.imageName}" onerror="this.src='${root}/static/css/noimage.jpg'"></li>
							...
							<li>
								<span style="cursor: pointer; color: #309cef;" onclick="myCouponDel('${couponList.purchaseCode}')">취소하기</span>
							</li>
						</ul>
					</c:when>
					<c:when test="${couponList.purchaseStatus == 'N'}">
		...
		<div id="myFavorite">
			...
			<div class="favorite_2">
				...
				<ul id="${favoriteList.foodCode}">
				  <li><img alt="음식점" src="/static/ftp/${favoriteList.imageName}"></li>
				...
				<ul id="${foodList.foodCode}">
				  ...
				  <c:choose>
						<c:when test="${foodList.foodStatus == 'N'}">
						  <li style="cursor: pointer; color: #309cef;" onclick="myFoodDel('${foodList.foodCode}')">취소</li>
						</c:when>
